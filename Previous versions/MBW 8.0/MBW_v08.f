...
C Calculating the magnitude of the deviatoric trial stress tensor
     ...
     1   **2 + two*sig6**2 )
C
C Preventing a divide by zero when dsmag is zero. When dsmag is zero, 
C computation is still in the elastic zone
...
C
C Eq. (13) - Determining cthetaax, either tension or compression
...
C Calculating equivalent plastic strain to damage initiation
        epsilondi = (half * ( Ddi1*exp(zero-Ddi2*eta) + 
     1  Ddi5*exp(zero-Ddi6*eta))-Ddi3*exp(zero-Ddi4*eta))*thetabar**2
     1  +half*(Ddi1*exp(zero-Ddi2*eta)-Ddi5*exp(zero-Ddi6*eta))                   
     1  *thetabar + Ddi3*exp(zero-Ddi4*eta)         
...
c			   stateNew(i,2) = stateOld(i,2) + Dcrit*(deqps/epsilondf)
			   stateNew(i,2) = Dcrit*stateNew(i,32)
            else
             stateNew(i,2) = one
			 stateNew(i,32) = one
			 stateNew(i,4) = three
