...
C Cleavage initiation locus
        ...
        Cc3    = props(35)
        Cc4    = props(36)	
C 		PROPS(37)=Switch temerapture function on(1)/of(0)
C  		PROPS(38)=Switch strain function on(1)/of(0)
...
C Calculating softening correction term due to temperature rise
        IF (ntempSwitch.EQ.zero) THEN 
		facT = one
		stateNew(i,18)= facT
		ELSE IF (tempOld(i).le.tol .and. tempOld(i).ge.-tol) THEN
