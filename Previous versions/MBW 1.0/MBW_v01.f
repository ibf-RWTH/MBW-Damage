...
C Computation per material point starts here
      do 100 i = 1,nblock
C
C BRITTLE DAMAGE OFF
C If brittle damage has previously occured, stress is zeroed
C        if (stateOld(i,4).gt.half.and.stateOld(i,4).lt.(one+half)) then
C          stressNew(i,1) = zero
C          stressNew(i,2) = zero
C          stressNew(i,3) = zero
C          stressNew(i,4) = zero
C          stressNew(i,5) = zero
C          stressNew(i,6) = zero
C          stateNew(i,4) = one
C          goto 100
C        endif
...
C       BRITTLE DAMAGE OFF
C       Brittle damage
C        if (sigmamax.ge.sigdmg .and. stressNew(i,1).gt.zero) then
C          stressNew(i,1) = zero
C          stressNew(i,2) = zero
C          stressNew(i,3) = zero
C          stressNew(i,4) = zero
