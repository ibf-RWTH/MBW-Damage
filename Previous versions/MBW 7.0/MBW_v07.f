...
C------------------------------------------------------------C
C            SDV and parameters added by Bo Wu               C
C SDV29:	Eta         								     C
C SDV30:	Thetabar										 C
C SDV31:	Damage initiation indicator						 C
C SDV32:	Failure indicator								 C
C SDV33:	damage initiation strain at the current step	 C
C SDV34:	Dcrit at the current step						 C
C       Additional parameters props(28)-props(32)            C
C Additional parameter for cut-off value is defined			 C
C Additional parameters Dc1-Dc4 are defined for Dcrit-locus	 C
C -----------------------------------------------------------C
C               Modifications by Bo Wu                       C
...
C -----------------------------------------------------------C
C            SDV and parameters added by Jinshan He          C
C SDV35:	Cleavega initiation strain at the current step	 C
C SDV36:	Cleavage failure indicator						 C
C       Additional parameters props(33)-props(36)            C
C Additional parameters Cc1-Cc4 are defined for Cleavage 	 C
C -----------------------------------------------------------C
C               Modifications by Jinshan He                  C
C Cleavage failure indicator: Cleavage happens when SDV36>1  C
C Non-proportional Loading paths with varying stress states  C
C are considered are considered with weighting scheme        C
C -----------------------------------------------------------C
C               Merged by Fuhui Shen                         C
C -----------------------------------------------------------C
        subroutine vumat(
C Read only
...
C Calculating equivalent plastic strain to damage initiation
        epsilonf = (half * ( Ddi1*exp(zero-Ddi2*eta) + Ddi5*exp(zero-Ddi6*eta))
     1                     - Ddi3*exp(zero-Ddi4*eta) )
     1                     * thetabar**2
     1           +  half * ( Ddi1*exp(zero-Ddi2*eta) - Ddi5*exp(zero-Ddi6*eta))
     1                     * thetabar
     1           +  Ddi3*exp(zero-Ddi4*eta)
          stateNew(i,33)= epsilonf
c  update damage initiation indicator, damage initition is suppressed when the stress triaxiality is less than the cut-off value
        if (eta.lt. Cut .and. stateNew(i,31).lt.1.) then
          stateNew(i,31) = stateOld(i,31)
        else
        stateNew(i,31) = stateOld(i,31)+deqps/epsilonf
        endif
...
C Calculating equivalent plastic strain to cleavage failure
     ...
     1           +  Cc3*exp(zero-Cc4*eta)
	   ...
	   stateNew(i,36) = stateOld(i,36)+stateNew(i,17)/epsilonc		
...
C         Updating failure indicator and damage fraction variable
        Dcrit=(Dc1*exp(zero-Dc2*eta) - Dc3*exp(zero-Dc4*eta) )
     1         * thetabar**2  +  Dc3*exp(zero-Dc4*eta)
          stateNew(i,34)= Dcrit
          stateNew(i,2) = stateOld(i,2) + stateNew(i,3)
     1                    /(gf) * deqps
          stateNew(i,32) = stateOld(i,32) + stateNew(i,3)
     1                    /(gf*Dcrit) * deqps
