...
C   function of both hydrostatic pressure and lode angle)-to-C
C - Damage evolution based on energy dissipated during       C
C   damage process -turned off-                              C
C                                                             C
C - Temperature softening            -turned off-             C
C - Strain rate hardening            -turned off-             C
C                                                            C
...
C Defining numerical constants
     ...
     1  dot1=4.96, dot2=6.42, dot3= 0.11, dot4=1.55) 
C
C
C
...
C          TEMP DEP OFF
C           facT = cT1*exp(zero-cT2*t0)+cT3
          facT = one
          stateNew(i,18)= facT
        else
C          TEMP DEP OFF
C          facT = cT1*exp(zero-cT2*tempOld(i))+cT3
          facT = one
          stateNew(i,18)= facT
        endif   
C
...
C       STRRATE DEP OFF
c        facStrrateM = cE1*log(stateOld(i,21)) + cE2
C        facStrrateA = cE3*stateOld(i,21)
        facStrrateM = one
        facStrrateA = zero
C
...
C Calculating the magnitude of the deviatoric trial stress tensor
        dsmag = sqrt( ds1**2 + ds2**2 + ds3**2 + two*sig4**2 + two*sig5
     1   **2 + two*sig6**2 )
C
...
C Calculating equivalent plastic strain to cleavage failure
       epsilonc=  ( dot1*exp(zero-dot2*eta) - 
     1                    dot3*exp(zero-dot4*eta))* thetabar**2
     1           +  dot3*exp(zero-dot4*eta)
       stateNew(i,31) = epsilonc
	   stateNew(i,32) = stateOld(i,32)+stateNew(i,17)/epsilonc
C
C Ductile damage
...
C         Updating damage fraction variable
          stateNew(i,2) = stateOld(i,2) + (charLength(i)*stateNew(i,3)
     1                    /(two*gf)) * deqps
          stateNew(i,4) = two
        else
C
...
C          DAMAGE OFF
C          facDctlDmgDev = zero
C          facDctlDmgVol = zero
          facDctlDmgDev = one
          facDctlDmgVol = one
          stateNew(i,2) = Dcrit
C             DAMAGE OFF
C         stateNew(i,28)= zero
        else
C            DAMAGE OFF
...
C       Brittle damage
      if (stateNew(i,32).ge.one .and. stateNew(i,1).gt.tol) then
